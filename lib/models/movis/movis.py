...
from .image_encoder import HybridEncoder
from .depth_predictor.ddn_loss.object_loss import OBJLoss
...
class MoVis(nn.Module):
    """ This is the MoVis module that performs monocualr 3D object detection """
    ...
    def __init__(self, backbone, depthaware_transformer, depth_predictor, num_classes, num_queries, num_feature_levels,
        ...
        self.img_encoder = HybridEncoder(in_channels=[512, 1024, 2048], feat_strides=[8, 16, 32], use_encoder_idx=[2])
    ...
    def forward(self, images, calibs, targets, img_sizes, dn_args=None):
        ...
        # for i in range(1):  # 可视化卷积相应的通道数量
        #     # 以下绘制了一个宽度为6，高度为1的展示区域
        #     # plt.subplot(6, 1, i + 1)
        #     plt.axis('off')
        #     # 制定使用jet热力图展示，还有其他的展示形式
        #     plt.imshow(images.contiguous().data.cpu().numpy()[5, 1, :, :], cmap='jet')
        #     # plt.savefig('./heatmap/{}.jpg'.format(i), bbox_inches='tight', pad_inches=0, dpi=1000)
        #     plt.show()
        #     # plt.imshow(proj_feats[enc_ind].data.cpu().numpy()[5, i, :, :], cmap='jet')
        #     # plt.savefig('./heatmap/{}.jpg'.format(i), bbox_inches='tight', pad_inches=0, dpi=1000)
        #     # plt.show()
        ...
        srcs, object_prods = self.img_encoder(srcs)
        ...
        #     plt.axis('off')
        #     # 制定使用jet热力图展示，还有其他的展示形式
        #     plt.imshow(srcs[-1].sum(dim=1).contiguous().data.cpu().numpy()[5, :, :], cmap='jet')
        #     plt.savefig('./3.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
        #     # plt.show()
        # exit()
        ...
        pred_depth_map_logits, depth_pos_embed, weighted_depth, depth_pos_embed_ip = self.depth_predictor(srcs, images,
                                                                                                          masks[1],
                                                                                                          pos[1])
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1], 'pred_3d_dim': outputs_3d_dim[-1],
               ...
               'pred_depth_map_logits': pred_depth_map_logits, 'pred_object_feature': object_prods}
...
class SetCriterion(nn.Module):
    """ This class computes the loss for MoVis.
    ...
    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, group_num=11):
        ...
        self.obj_loss = OBJLoss()  # for vision map
    ...
    def loss_object_feature(self, outputs, targets, indices, num_boxes):
        object_feature = outputs['pred_object_feature']

        num_gt_per_img = [len(t['boxes']) for t in targets]
        gt_boxes2d = torch.cat([t['boxes'] for t in targets], dim=0) * torch.tensor([40, 12, 40, 12], device='cuda')
        gt_boxes2d = box_ops.box_cxcywh_to_xyxy(gt_boxes2d)
        gt_center_depth = torch.cat([t['depth'] for t in targets], dim=0).squeeze(dim=1)

        losses = dict()

        losses["loss_object_map"] = self.obj_loss(
            object_feature, gt_boxes2d, num_gt_per_img, gt_center_depth)
        return losses
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        ...
        loss_map = {
            ...
            'object': self.loss_object_feature,  # 添加
        }
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss == 'depth_map' or loss == 'object':
...
def build(cfg):
    ...
    weight_dict['loss_object_map'] = cfg['object_feat_loss_coef']
    ...
    losses = ['labels', 'boxes', 'cardinality', 'depths', 'dims', 'angles', 'center', 'depth_map', 'object']
