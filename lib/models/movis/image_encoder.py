...
class SpaceAttention(nn.Module):
    def __init__(self, in_channel, d_model):
        super().__init__()
    ...
    def forward(self, x, v):
        ...
        # x_h = self.avgpool_h(x).squeeze().permute(0, 2, 1)
        # x_w = self.avgpool_w(x).squeeze().permute(0, 2, 1)
        q = self.mlp(x_h)
        k = self.mlp(x_w).transpose(-1, -2)

        weight_score = torch.matmul(q, k)
        weight_probs = F.sigmoid(weight_score).unsqueeze(1)
...
class CSPRepLayer(nn.Module):
    def __init__(self,
        ...
        self.space_att = SpaceAttention(in_channels, out_channels)
    ...
    def forward(self, feats):
        ...
        if self.num_encoder_layers > 0:
            for i, enc_ind in enumerate(self.use_encoder_idx):
                ...
                # object map
                object_logits = self.object_head(proj_feats[enc_ind]) + self.res_block(proj_feats[enc_ind])
                # object_probs = F.softmax(object_logits)
                object_probs = object_logits
                ...
                proj_feats[enc_ind] = memory.permute(0, 2, 1).reshape(-1, self.hidden_dim, h,
                                                                      w).contiguous() + object_probs
                ...
                #     plt.imshow(proj_feats[enc_ind].sum(dim=1).contiguous().data.cpu().numpy()[5, :, :], cmap='jet')
                #     # plt.savefig('./att.jpg', bbox_inches='tight', pad_inches=0, dpi=1200)
                #     # np.savetxt('./1.txt', proj_feats[enc_ind].sum(dim=1).contiguous().data.cpu().numpy()[5, :, :])
                #     plt.show()
                # exit()
